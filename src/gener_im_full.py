...
def render_single_date(data: dict, day_ts: int, day_key: str, output_filename: str, date_str: str):
    ...
    # --- Заголовок ---
    # Конвертируем дату в украинский формат
    dt = datetime.fromtimestamp(day_ts, ZoneInfo('Europe/Kyiv'))
    ukrainian_date = f"{dt.day} грудня"
    title_text = f"Графік погодинних відключень на {ukrainian_date}"
    ...
    for state in legend_states:
        ...
        text_y = legend_y_start + box_size // 2  # Центр квадрата
